...
class OcclusionLayer(nn.Module):
    def __init__(self, image):
        ...
        self.fc1 = OcclusionFirstLayer(size_in=5, size_out=image_height * 2 + image_width * 2 + 1)
        self.fc2 = OcclusionSecondLayer(size_in=self.fc1.size_out, size_out=(self.fc1.size_out - 1) // 2 + 1)
        self.fc3 = OcclusionThirdLayer(size_in=self.fc2.size_out, size_out=image_width * image_height * 2, image_shape=image.shape)
        self.fc4 = OcclusionFourthLayer(size_in=self.fc3.size_out, size_out=image_channel * image_width * image_height, image=image)
...
class OcclusionThirdLayer(nn.Module):
    ...
    def init_weights_bias(self, size_in, size_out, image_shape):
        ...
        # output has 2 part, one for occlusion and one for occlusion color
        for i in range(2):
            for j in range(block_size):
                r, c = j // image_width, j % image_width
                weights[i * block_size + j, r] = 100
                weights[i * block_size + j, input_block_size + c] = 100
                if i == 1:
                    weights[i * block_size + j, -1] = 1

        bias[0:block_size] = -torch.ones(block_size) * 199
        bias[block_size:] = -torch.ones(block_size) * 200
    ...
    def init_weights_bias(self, size_in, size_out, image):
        ...
        for channel in range(image_channel):
            for i in range(size_out // image_channel):
                ...
                weights[channel * image_height * image_width + i, size_in // 2 + i] = 1
        bias = torch.ones(size_out)
