...
from gtsrb.occlusion_layer_extend.occlusion_layer_v3 import OcclusionLayer
...
def restore_extended_model():
    ...
    occlusion_layer = OcclusionLayer(image=fake_image)
    extended_model = nn.Sequential(
        occlusion_layer,
        *list(model.children())
    )
    extended_model.load_state_dict(torch.load('../../model/extended/fnn_model_gtsrb_small_extended.pth', map_location=torch.device('cpu')))
