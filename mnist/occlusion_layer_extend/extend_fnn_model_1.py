...
def save_extended_model():
    ...
    torch.save(extended_model.state_dict(), '../../model/extended/v3.1/fnn_model_mnist_1_extended_shrink.pth')
...
def save_extended_model_onnx():
    ...
    onnx_model_filename = 'fnn_model_mnist_1_extended_shrink.onnx'
    torch.onnx.export(extended_model, dummy_input, '../../model/extended/v3.1/' + onnx_model_filename)


def restore_and_run_with_marabou():
    network = Marabou.read_onnx('../../model/extended/v3.1/fnn_model_mnist_1_extended_shrink.onnx')
...
def restore_and_verify_with_marabou():
    ...
    network.setUpperBound(inputs[0], 28)
    network.setLowerBound(inputs[1], 1)
    network.setUpperBound(inputs[1], 20)
    network.setLowerBound(inputs[2], 1)
    network.setUpperBound(inputs[2], 28)
    network.setLowerBound(inputs[3], 1)
    network.setUpperBound(inputs[3], 20)
