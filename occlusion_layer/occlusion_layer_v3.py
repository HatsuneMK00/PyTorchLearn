...
class OcclusionLayer(nn.Module):
    def __init__(self, image, occlusion_color, first_layer, is_cnn=False):
        ...
        self.fc4 = OcclusionFourthLayer(size_in=self.fc3.size_out, size_out=image_channel * image_width * image_height, image=image, occlusion_color=occlusion_color, model_first_layer=first_layer, is_cnn=is_cnn)
...
class OcclusionFourthLayer(nn.Module):
    def __init__(self, size_in, size_out, image, occlusion_color, model_first_layer, is_cnn):
        super().__init__()
        self.size_in = size_in
        if not is_cnn:
