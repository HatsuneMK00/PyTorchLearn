...
class OcclusionLayer(nn.Module):
    def __init__(self, image, first_layer, shrink=True):
        ...
        self.fc4 = OcclusionFourthLayer(size_in=self.fc3.size_out, size_out=image_channel * image_width * image_height, image=image, model_first_layer=first_layer, shrink=shrink)
...
class OcclusionThirdLayer(nn.Module):
    ...
    def init_weight_bias_for_epsilons(self, size_in_eps, size_out, image_shape):
        ...
        _, image_height, image_width = image_shape
        input_block_size = size_in_eps // 2
        for i in range(size_out):
            r, c = (i // 2) // image_width, (i // 2) % image_width
            if i % 2 == 0:
                weights[i, r] = 1
...
class OcclusionFourthLayer(nn.Module):
    def __init__(self, size_in, size_out, image, model_first_layer, shrink):
        super().__init__()
        self.size_in = size_in
        if shrink:
